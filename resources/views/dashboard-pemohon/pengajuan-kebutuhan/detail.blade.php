                ...
                <div class="card-body">
                    <div class="row justify-content-md-end" style="align-items: center">
                        ...
                        <a target='_blank'
                            ...
                            style='position:absolute; width:130px; right:40px; top:110px;' class='btn btn-warning'>
                            <i class="fa-solid fa-print fa-lg"></i> Cetak Data
                        </a>
                    </div>
                    <div id='Ajukan'>
                        @if (
                            $pengajuan_kebutuhan->status == 'Terkirim' ||
                            ...
                            <button disabled idPengajuanKebutuhan='{{ $pengajuan_kebutuhan->id_pengajuan_kebutuhan }}'
                                class='btn btnAjukan btn-secondary fw-bold'
                                style='letter-spacing:1px; cursor:default; position:absolute; right:40px; top:20px;'><i
                                    class="fa-solid fa-paper-plane"></i> Ajukan</button>
                        @else
                            <button idPengajuanKebutuhan='{{ $pengajuan_kebutuhan->id_pengajuan_kebutuhan }}'
                                class='btn btnAjukan btn-success fw-bold'
                                style='letter-spacing:1px; position:absolute; right:40px; top:20px;'><i
                                    class="fa-solid fa-paper-plane"></i> Ajukan</button>
                        @endif
                    </div>
                            ...
                            <tbody>
                                ...
                                @foreach ($item_kebutuhan as $p)
                                    <tr>
                                        ...
                                        <td>
                                            @if ($pengajuan_kebutuhan->status == 'Draf')
                                                <a href="/dashboard-pemohon/item-kebutuhan/edit/{{ $p->id_item_kebutuhan }}"
                ...
                $('#Ajukan').on('click', '.btnAjukan', function(a) {
                    ...
                    swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                type: 'POST',
                                url: '/dashboard-pemohon/pengajuan-kebutuhan/ajukan/' +
                                ...
                                success: function(data) {
                                    if (data.success) {
                                        swal.fire('Pengajuan kebutuhan Berhasil Diajukan!', '',
